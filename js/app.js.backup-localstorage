 ...
 * Version 3.0 - Batch Print, Monthly Report, Activity Log
 */
...
// ==================== //
// LocalStorage - Registry Data
// ==================== //

const STORAGE_KEY_REGISTRY = 'boi_registry_data';
const STORAGE_KEY_PRINTED = 'boi_printed_receipts';
const STORAGE_KEY_RECEIVED = 'boi_received_cards';
const STORAGE_KEY_ACTIVITY = 'boi_activity_log';
...
function saveRegistryToStorage() {
    try {
        localStorage.setItem(STORAGE_KEY_REGISTRY, JSON.stringify(state.registryData));
    } catch (e) {
...
function loadActivityLog() {
    try {
        ...
        state.activityLog = stored ? JSON.parse(stored) : [];
    } catch (e) {
...
function saveActivityLog() {
    try {
        ...
        localStorage.setItem(STORAGE_KEY_ACTIVITY, JSON.stringify(state.activityLog));
    } catch (e) {
...
function addActivity(type, title, details = '') {
    const activity = {
        id: Date.now(),
        type: type, // 'add', 'edit', 'delete', 'print', 'receive'
        title: title,
        details: details,
...
function clearActivityLog() {
    if (!confirm('ต้องการล้าง Activity Log ทั้งหมดหรือไม่?')) return;
    state.activityLog = [];
    saveActivityLog();
    renderActivityLog();
}
    ...
    return `
        <div class="print-receipt-page" style="font-family: 'Sarabun', sans-serif; font-size: 14px; line-height: 1.4; padding: 10mm 15mm;">
            ...
            <div style="margin-top: 25px; display: flex; justify-content: space-between; border-top: 1px solid #e5e7eb; padding-top: 10px;">
                ...
                <span style="color: #9ca3af; font-size: 10px;">Doc No.: ${formData.receiptNo || '-'}</span>
            </div>
        </div>
    `;
}
...
function clearForm(skipConfirm = false) {
    ...
    setDefaultDate();
    const newReceiptNo = generateNextReceiptNo(state.registryData);
...
function saveData() {
    ...
    if (state.formMode === 'edit') {
        ...
        if (exists) {
            alert('เลขรับที่นี้มีอยู่แล้ว กรุณาใช้เลขรับที่อื่น');
            return;
        }

        // Add new record
        newRecord.number = state.registryData.length + 1;
        state.registryData.push(newRecord);
    ...
    renderRegistryTable();
    updateSummary();
    clearForm(true); // Skip confirmation after successful save
}
...
async function loadRegistryData() {
    ...
    const storedData = loadRegistryFromStorage();

    if (storedData.length > 0) {
        state.registryData = storedData;
    } else {
        // Use mock data if no stored data
        state.registryData = [
            { number: 1, receiptNo: '6902/0001', sn: 'SN001234567', name: 'RAFAEL MASSAYOSHI NIITSUMA', date: '4/2/2569', requestNo: 'B69113800005141', appointmentNo: '1-BKK001022600508-SW', cardImage: null },
            { number: 2, receiptNo: '6902/0002', sn: 'SN001234568', name: 'JOHN WILLIAM SMITH', date: '4/2/2569', requestNo: 'B69113800005147', appointmentNo: '1-BKK001022600514-SW', cardImage: null },
            { number: 3, receiptNo: '6902/0003', sn: 'SN001234569', name: 'MARIA GARCIA LOPEZ', date: '4/2/2569', requestNo: 'B69113800005146', appointmentNo: '1-BKK001022600513-SW', cardImage: null },
        ];
        saveRegistryToStorage();
    }
...
function applyPermissions() {
    ...
    const session = window.AuthSystem.getSession();
    if (!session) return;
    ...
    if (!window.AuthSystem.hasPermission('activity_log')) {
    ...
    if (!window.AuthSystem.hasPermission('monthly_report')) {
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('Work Permit Receipt System v4.0 initialized');

    // Check authentication - wait a bit for AuthSystem to be ready
    setTimeout(() => {
...
async function initializeApp() {
    ...
    setupEventListeners();
    setupUserManagement();
    setupSheetImport(); // Setup Google Sheets import

    // Load registry data and WAIT for it to complete
    await loadRegistryData();

    // Set default date and auto-generate receipt number AFTER data is loaded
    setDefaultDate();
    const newReceiptNo = generateNextReceiptNo(state.registryData);
    elements.receiptNo.value = newReceiptNo;
    ...
    // Initialize monthly report dropdowns
    ...
    // Pre-load sheet data in background
    setTimeout(() => {
        fetchSheetData();
    }, 1000);
}
